...
# 1. Use snscrape to get raw tweets (which will include retweets) because we want to see if there's any new users we should add to our list. 
    # Tweepy is more consistent - sometimes snscrape cannot find a guest token - but I wasn't able to find a way to pull native retweets using Tweepy 
...
# 3. Run a full serach of X tweets for your combined master list of users, using tweepy, which will have more attributes than snscrape 
...
# 6. Lastly, apply NLP to the original master list of tweets, pull out the stop words, and see what are the most common words being used in these tweets. 
# 7. Use the information to run a phrase-based query in Twitter. You might also have an existing list of text terms, and then could use the additional phrases as an extension 
    # similar to how you had a list of users initially, but then added some new users based on the retweets. 
# 7a. Write a CSV with all tweets (organized in a dataframe) so you can look at it later. 
...
initial_phrase_list = ['christmas cheer', 'holiday kindness', 'kinship'] # first used in step # 6 
master_phrase_list = [] # first used in step # 6 

US_tweets = []
ASIA_tweets = [] 
final_text = []
    # this is to be used in our final step, # 8, when we print an outline of the weekly draft, but we are not printing it here. This is first used in step # 4 
master_tweets = []
    # creating an empty list to house all of your tweets, before putting them in a Dataframe - first used in #3 
master_user_list = []
    # this is to house the adding of two lists - first used in # 2 
new_user_list = []
    # this is to house the additional users you found - first used in # 2 
first_users_list = ['HEB', 'DalaiLama','jnovogratz','FredRogersPro','Yoyo_Ma','somegoodnews','goodgoodgood','goodnewsmoveme3','withlovecleo'] 
    # first used in part # 1 

# 8 --- let's create the final previes 
...
def weekly_preview():
    ...
    print("\n")
    print(f"Our favorite story is {favorite_story}") #you might want to make this manual -- have Python prompt you to select or type in the tweet 
    print("\n")
    ...
    print("\n")
    print(f"We particularly liked some stories that focused on {master_phrase_list}") # and our picks for this week are [J,K,L], where [J,K,L] should be the top stories from a sorted [phrase_search_tweets]
    print("\n")
    print("For a heartwarming longer read, check out [Y]")
    print("\n")
    ...
    final_CSV.close()
    # this CSV file needs work -- the columns are not aligned because you are running two searches with different columns, rework step #7 to use Tweepy
    
    # https://www.learnbyexample.org/reading-and-writing-csv-files-in-python/
    #print("<sentence>.\n<second sentence>.\n") -- syntax for if you want to combine everything ^^ into one print statement 
    exit() 
...
def text_based_search():
    ...
    for phrase in master_phrase_list:
        for i,tweet in enumerate(sntwitter.TwitterSearchScraper(f'{phrase}').get_items()):
            if i>10:
                break
            phrase_search_tweets.append([tweet.date, tweet.content, tweet.user.username, tweet.likeCount, tweet.retweetCount])
            # .extend did not work here, the snscrape search results in the shape being (44,1) not (44,4) 
    
    df_last_frame = pd.DataFrame(phrase_search_tweets, columns=['Datetime', 'Text', 'Username','Likes','Retweets'])
    print(df_last_frame)
    ...
    # do we want to amend the colums we have here? We should at least match the search we made on Tweepy earlier. If possible, try to use Tweepy for this function as well / it's cleaner that way 
...
def top_tweets_per_location():
    ...
    df=pd.DataFrame(master_tweets,columns=['ID','Created','Text','Location','Retweet Count','Likes'])
    
    # try creating a new column to show the broader region its part of 
    df['Global Region'] = df['Location'].map({'San Antonio, Texas' : 'United States', 'Dharamsala, India' : 'Asia', 'New York, NY': 'United States', 'Pittsburgh' : 'United States'})
    ...
    df_USA_sorted=df_USA.sort_values(["Retweet Count"],ascending=(False))
    ...
    df_ASIA_sorted=df_ASIA.sort_values(["Retweet Count"],ascending=(False))
...
def popular_rank():
    ...
    df=df.sort_values(["Retweet Count"],ascending=(False))
...
def full_search():
    ...
    for master_user in master_user_list:
        ...
        outtweets = [[info.id, info.created_at, info.full_text, info.user.location, info.favorite_count, info.retweet_count] for info in tweets]
        master_tweets.extend(outtweets)
    
    df=pd.DataFrame(master_tweets,columns=['ID','Created','Text','Location','Likes','Retweet Count'])
    
    fts = open('full_tweet_search.csv', 'w', encoding='utf-8')
    writer = csv.writer(fts) 
    writer.writerows(master_tweets) 
    fts.close()
    # let's modify this file at the end, but I think it helps to create the full_tweet_search in the interim so that we can open the file/look at it to input phrases as well 
...
def add_users(): 
    ...
    while True: 
        ...
        if new_user == 'no one':
            ...
            for i,twit in enumerate(sntwitter.TwitterSearchScraper(f'from:{new_user}, include:nativeretweets').get_items()): 
                if i>10: 
                    break
                tweets_list2.append([twit.content, twit.likeCount, twit.retweetCount, twit.mentionedUsers]) 
            tweets_df2 = pd.DataFrame(tweets_list2, columns=['Text', 'Likes', 'Retweets', 'Mentioned Users'])
            print(tweets_df2)
...
def intro():
    ...
    for user in first_users_list: 
        for i,tweet in enumerate(sntwitter.TwitterSearchScraper(f'from:{user}, include:nativeretweets').get_items()): #declare a username 
            if i>10: #number of tweets you want to scrape
                break
            tweets_list1.append([tweet.user, tweet.content, tweet.likeCount, tweet.retweetCount, tweet.mentionedUsers]) 
        
    tweets_df1 = pd.DataFrame(tweets_list1, columns=['User', 'Text','Likes','Retweets','Mentioned Users'])
    print(tweets_df1)
    # I need a CSV in this case because I'm not working in Jupyter notebook so I can't see the entire rows/columns, but this part is optional
